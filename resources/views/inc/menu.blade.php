                    ...
                    <div class="collapse navbar-collapse">
                        <ul class="navbar-nav nav-menu-ul">
                            ...
                            <li class="nav-menu-btn">
                                <a class="btn-menu text-uppercase nav-a" id="dropdown{{$category->id}}"
                                    data-toggle="dropdown" aria-haspopup="true"
                                    aria-expanded="false"><small>{{$category->title}}</small></a>
                                <div class="dropdown-menu megamenu rounded-0"
                                    aria-labelledby="dropdown{{$category->id}}">
                                    <div class="row">
                                        @foreach (App\Category::where('parent_id', $category->id)->get() as $child)
                                        <div class="col-md-4 col-lg-3">
                                            <a class="dropdown-item text-uppercase text-teal"
                                                href="/c/{{$child->slug}}"><span
                                                    class="text-teal">{{$child->title}}</span></a>
                                            @foreach (App\Category::where('parent_id', $child->id)->get() as
                                            $grand_child)
                                            <a class="dropdown-item nav-link" href="/c/{{$grand_child->slug}}"><span
                                                    class="text-pink ml-3">--- </span>{{ $grand_child->title }}</a>
                                            @endforeach
                                        </div>
                                        @endforeach
                                    </div>
